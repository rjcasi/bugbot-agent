const pattern17Bridge = new Pattern({
  id: 17,
  name: "Bridge",
  ...
  slots: {
    source_domain:
      "Where the signal originates: symbolic_layer, cyber_panel, spike_tensor, effector_system, phase_space, memory_core.",
    target_domain:
      "Where the signal is mapped: robotics_layer, meaning_core, anomaly_detector, embedding_space, cyber_defense, phase_orbit.",
    mapping:
      "How translation occurs: projection, alignment, normalization, symbolic_translation, cross_modal_mapping, energy_preserving_transform."
  },
  examples: [
    {
...
PATTERNS[pattern17Bridge.id] = pattern17Bridge;


function renderBridge(source_domain, target_domain, mapping) {
  return `BRIDGE ${source_domain} -> ${target_domain} via ${mapping}`;
}