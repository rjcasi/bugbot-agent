pattern_21_conduit = Pattern(
    ...
    slots={
        "source": (
            "Where the signal originates: spike_origin, symbolic_core, cyber_detector, "
            "effector_sensor, memory_node, phase_cell."
        ),
        "pathway": (
            "The channel: direct_line, filtered_channel, shielded_route, energy_path, "
            "symbolic_pipe, cyber_tunnel."
        ),
        "destination": (
            "Where the signal is delivered: effector_motor, meaning_core, anomaly_resolver, "
...
PATTERNS[pattern_21_conduit.id] = pattern_21_conduit


def render_conduit(source: str, pathway: str, destination: str) -> str:
    """
    Render a Conduit pattern instance using the canonical structure.
    """
    return f"CONDUIT {source} via {pathway} -> {destination}"