const pattern14Weave = new Pattern({
  id: 14,
  name: "Weave",
  ...
  slots: {
    threads:
      "The streams being combined: spikes+embeddings, cyber_logs+intention, effector_pose+phase_space, fuzz+defense.",
    loom:
      "The integration method: weighted_sum, attention_map, symbolic_binding, phase_alignment, cross_entropy_fusion, temporal_sync.",
    pattern:
      "The resulting unified structure: fused_embedding, coherent_action, unified_signal, stabilized_behavior."
  },
  examples: [
    {
...
PATTERNS[pattern14Weave.id] = pattern14Weave;


function renderWeave(threads, loom, pattern) {
  return `WEAVE ${threads} via ${loom} -> ${pattern}`;
}