const pattern26Matrix = new Pattern({
  id: 26,
  name: "Matrix",
  ...
  slots: {
    axes:
      "The dimensions: time_axis, meaning_axis, energy_axis, cyber_axis, spatial_axis, phase_axis.",
    mapping:
      "The interaction rule: orthogonal, aligned, weighted, cross_product, tensor_map, symbolic_projection.",
    cell:
      "The resulting unit: meaning_cell, spike_cell, cyber_cell, effector_cell, memory_cell, phase_cell."
  },
  examples: [
    {
...
PATTERNS[pattern26Matrix.id] = pattern26Matrix;


function renderMatrix(axes, mapping, cell) {
  return `MATRIX ${axes} via ${mapping} -> ${cell}`;
}