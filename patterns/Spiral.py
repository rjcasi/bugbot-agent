pattern_25_spiral = Pattern(
    ...
    slots={
        "origin": (
            "The starting point: symbolic_seed, spike_origin, cyber_root, "
            "effector_base, memory_anchor, phase_source."
        ),
        "recursion": (
            "The transformation rule: amplify, refine, expand, rotate, elevate, compress."
        ),
        "ascent": (
            "The upward dimension: meaning_depth, energy_level, phase_height, "
            "symbolic_layer, cyber_complexity, spatial_scale."
        )
    },
    examples=[
        {
...
PATTERNS[pattern_25_spiral.id] = pattern_25_spiral


def render_spiral(origin: str, recursion: str, ascent: str) -> str:
    """
    Render a Spiral pattern instance using the canonical structure.
    """
    return f"SPIRAL {origin} via {recursion} toward {ascent}"