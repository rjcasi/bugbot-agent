const pattern22Bloom = new Pattern({
  id: 22,
  name: "Bloom",
  ...
  slots: {
    seed:
      "The initial element: symbolic_seed, spike_pattern, cyber_signature, effector_idea, memory_fragment, phase_kernel.",
    growth_rule:
      "The expansion logic: recursive, branching, harmonic, exponential, fractal, diffusion.",
    field:
      "The domain of proliferation: meaning_field, spike_field, cyber_field, effector_field, memory_field, phase_field."
  },
  examples: [
    {
...
PATTERNS[pattern22Bloom.id] = pattern22Bloom;


function renderBloom(seed, growth_rule, field) {
  return `BLOOM ${seed} via ${growth_rule} into ${field}`;
}