pattern_18_pulsefield = Pattern(
    ...
    slots={
        "nodes": (
            "The participating regions: spike_layers, cyber_organs, effector_clusters, "
            "symbolic_regions, phase_space_cells, memory_blocks."
        ),
        "waveform": (
            "The rhythmic pattern: sine, square, saw, burst, harmonic, chaotic_rhythm."
        ),
        "coupling": (
            "How nodes influence each other: phase_lock, entrainment, resonance, "
            "inhibitory_coupling, excitatory_coupling, diffusion."
        )
    },
    examples=[
        {
...
PATTERNS[pattern_18_pulsefield.id] = pattern_18_pulsefield


def render_pulsefield(nodes: str, waveform: str, coupling: str) -> str:
    """
    Render a Pulsefield pattern instance using the canonical structure.
    """
    return f"PULSEFIELD {nodes} with {waveform} via {coupling}"