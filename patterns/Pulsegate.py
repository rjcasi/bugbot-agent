pattern_32_pulsegate = Pattern(
    ...
    slots={
        "signal": (
            "The gated flow: spike_train, cyber_flow, symbolic_wave, effector_command, "
            "memory_stream, phase_wave."
        ),
        "interval": (
            "The timing: 50ms, 1s, harmonic, phase_locked, burst_window, "
            "duty_cycle."
        ),
        "condition": (
            "The gating rule: allow_if_stable, allow_if_clean, allow_if_resonant, "
            "allow_if_safe, allow_if_low_entropy, allow_if_trusted."
        )
    },
    examples=[
        {
...
PATTERNS[pattern_32_pulsegate.id] = pattern_32_pulsegate


def render_pulsegate(signal: str, interval: str, condition: str) -> str:
    """
    Render a Pulsegate pattern instance using the canonical structure.
    """
    return f"PULSEGATE {signal} at {interval} -> {condition}"