const pattern23Refine = new Pattern({
  id: 23,
  name: "Refine",
  ...
  slots: {
    input:
      "What is being improved: spike_sequence, symbolic_structure, cyber_trace, effector_motion, embedding_vector, phase_path.",
    criterion:
      "The improvement metric: minimize_error, maximize_alignment, reduce_entropy, increase_precision, stabilize_phase, sharpen_signal.",
    iteration:
      "The refinement method: gradient_step, recursive_pass, smoothing, filtering, correction_loop, alignment_pass."
  },
  examples: [
    {
...
PATTERNS[pattern23Refine.id] = pattern23Refine;


function renderRefine(input, criterion, iteration) {
  return `REFINE ${input} by ${criterion} via ${iteration}`;
}