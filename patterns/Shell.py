pattern_24_shell = Pattern(
    ...
    slots={
        "core": (
            "The protected element: symbolic_core, spike_generator, cyber_kernel, "
            "effector_brain, memory_seed, phase_anchor."
        ),
        "boundary": (
            "The protective layer: hard_shell, soft_shell, adaptive_shell, "
            "energy_barrier, symbolic_membrane, cyber_firewall."
        ),
        "policy": (
            "The boundary rule: allow_internal_only, block_entropy>0.5, "
            "preserve_integrity, non_destructive, read_only, anomaly_block."
        )
    },
    examples=[
        {
...
PATTERNS[pattern_24_shell.id] = pattern_24_shell


def render_shell(core: str, boundary: str, policy: str) -> str:
    """
    Render a Shell pattern instance using the canonical structure.
    """
    return f"SHELL {core} with {boundary} :: {policy}"