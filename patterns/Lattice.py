pattern_20_lattice = Pattern(
    ...
    slots={
        "nodes": (
            "The elements being connected: spike_cells, cyber_organs, symbolic_units, "
            "effector_points, memory_blocks, phase_space_cells."
        ),
        "links": (
            "The relationships: bidirectional, weighted, inhibitory, excitatory, "
            "symbolic_link, energy_link."
        ),
        "topology": (
...
PATTERNS[pattern_20_lattice.id] = pattern_20_lattice


def render_lattice(nodes: str, links: str, topology: str) -> str:
    """
    Render a Lattice pattern instance using the canonical structure.
    """
    return f"LATTICE {nodes} via {links} -> {topology}"