const pattern9Mirror = new Pattern({
  id: 9,
  name: "Mirror",
  ...
  slots: {
    source:
      "What is being reflected: spikes, embeddings, cyber logs, effector telemetry, symbolic intention, phase-space trajectory.",
    frame:
      "The interpretive lens: temporal, symbolic, geometric, ethical, cyber, or energy-preserving frame.",
    delta:
      "The measured difference between reality and expectation; alignment, drift, error, anomaly score."
  },
  examples: [
    {
...
PATTERNS[pattern9Mirror.id] = pattern9Mirror;


function renderMirror(source, frame, delta) {
  return `REFLECT ${source} :: ${frame} => ${delta}`;
}